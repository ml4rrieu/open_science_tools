...
"""
STEP 1 from openapc database  produce table with year, journals, and average of apc that have been payed 
...
issns_a = df["issn"].tolist()
issn_price = dict.fromkeys(issns_a) #remove duplicate
print(f"nb of non hybrid journals finded\t{len(issn_price)}\n\n")

for i, item in enumerate(issn_price) : 
	issn_price[item] = []
	for year in range(2015, 2021):
		mean = df.loc[ (df["issn"] == item) & (df["period"] == year), "euro" ].mean()
...
out.to_csv("./source/temp_journals_in_openapc.csv") # !!caution!! the issn column is the index :/
exit()
"""


#____STEP 2____ fill NA values if appropriate
df = pd.read_csv("./source/temp_openapc_journals.csv")
...
openapc = pd.read_csv("./source/openapc.csv", usecols=["issn", "issn_print", "issn_electronic", "journal_full_title"])
openapc.drop_duplicates(subset="issn", inplace = True)
journals = journals.merge(openapc, how="left", on= "issn")

#reorder columns
temp_col.insert(0, "journal_full_title")
temp_col.extend(["issn", "issn_print", "issn_electronic"])
